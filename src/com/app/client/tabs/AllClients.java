...
public class AllClients extends AppTab {

	private Record m_objectInfo;
	private ListGrid m_grid;

	public AllClients(Record objectInfo) {
		super();
		m_objectInfo = objectInfo;
		setTitle(img16("all_clients"), objectInfo.getAttribute("MAP_NAME") + " - " + objectInfo.getAttribute("NAME"));
	}

	protected Canvas getContent() {

		m_grid = new ListGrid(DS.PM_PORT_CLIENTS, true, false, false, false, true, true) {
			@Override
		...
		m_grid.addButton(img24("save_list"), G.S.saveList(), new ClickHandler() {
			public void onClick(ClickEvent event) {
				m_grid.exportToExcel("clients.xls");
			}
		});
		m_grid.setFieldStateId("GridAllClients");
