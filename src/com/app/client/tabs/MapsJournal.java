...
public class MapsJournal extends AppTab {

	public MapsJournal() {
		super();
		setTitle(img16("changes_journal"), G.S.mapsChangesJournal());
	}

	protected Canvas getContent() {

		final ListGrid grid = new ListGrid(DS.H_MAPS, true, false, false, false, true, true) {
			private final String[] compareFieldPairs = new String[] { "OLD_NAME", "NEW_NAME", "OLD_COMMENT", "NEW_COMMENT" };
		...
		grid.setHeaderSpans(
				new HeaderSpan(G.S.oldValues(), new String[] { "OLD_NAME", "OLD_COMMENT" }),
				new HeaderSpan(G.S.newValues(), new String[] { "NEW_NAME", "NEW_COMMENT" }));
		grid.setFieldStateId("GridMapsJournal");
